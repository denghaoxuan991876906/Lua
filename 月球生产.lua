...
configs:
  ...
  职业循环列表:
    description: |
      "当达到经验值或职业积分阈值时循环切换的职业列表（具体取决于ICE中的设置）。输入职业简称或全称并按回车，每行一个职业。请在Simple Tweaks插件中启用切换职业命令并保持其为默认设置。留空则禁用职业循环功能"
    default: []
  自动换取法恩娜探索计划证书:
    description: |
  ...
  雇员探索委托处理:
    description: |
      "当雇员探索完成时暂停宇宙任务。选择琉璃星或憧憬湾离开后不会返回月面基地。选择停用可关闭此功能"
    default: "N/A"
    is_choice: true
    choices: ["N/A","琉璃星", "憧憬湾"]
  研究点数缴纳:
  ...
  肝武职业循环:
    description: |
      "当肝武工具完成后循环切换的职业列表。不需包含起始当前职业，从下一个目标职业开始列出。输入职业简称或全称并按回车，每行一个职业。请在Simple Tweaks插件中启用切换职业命令并保持其为默认设置。留空则禁用职业循环功能"
    default: []
[[End Metadata]]
--]=====]
--[[
