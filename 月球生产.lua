...
function ShouldCredit()
    ...
    if lunarCredits >= LimitConfig and Svc.Condition[CharacterCondition.normalConditions] and not Player.IsBusy then
        ...
        -- 记录日志并输出提示
        Dalamud.Log("[Cosmic Helper] 月球信用点: " .. tostring(lunarCredits) .. "/" .. LimitConfig .. " 是时候转转盘了!")
        yield("/echo 月球信用点: " .. tostring(lunarCredits) .. "/" .. LimitConfig .. " 是时候转转盘了!")
        ...
        if Svc.ClientState.TerritoryType == SinusTerritory then  -- 憧憬湾
            ...
            Dalamud.Log("[Cosmic Helper] 去找环行威")
            sleep(1)
            -- 监控导航状态，到达附近后停止
            while IPC.vnavmesh.PathfindInProgress() or IPC.vnavmesh.IsRunning() do
                sleep(.02)
                ...
                if DistanceBetweenPositions(curPos, SinusCreditNpc.position) < 5 then
                    Dalamud.Log("[Cosmic Helper] 距离环行威够近了，停止导航.")
                    IPC.vnavmesh.Stop()
                end
            end
        elseif Svc.ClientState.TerritoryType == PhaennaTerritory then  -- 琉璃星
        ...
        if e then
            Dalamud.Log("[Cosmic Helper] 选中: " .. SinusCreditNpc.name)
            e:SetAsTarget()
        end
        if Entity.Target and Entity.Target.Name == SinusCreditNpc.name then
            Dalamud.Log("[Cosmic Helper] 交互: " .. SinusCreditNpc.name)
            e:Interact()
            sleep(1)
        end
        ...
        if IsAddonReady("SelectString") then
            yield("/callback SelectString true 0")  -- 再次选择第一个选项
            sleep(1)
        end
        ...
        while Svc.Condition[CharacterCondition.occupiedInQuestEvent] do
            sleep(1)
            Dalamud.Log("[Cosmic Helper] 等待转盘结束")
        end
